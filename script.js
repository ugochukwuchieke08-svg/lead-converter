...
function openModal(car) {
  ...
  modalImg.addEventListener("touchend", e => { const endX = e.changedTouches[0].clientX; if (endX - startX > 50) currentImg = (currentImg - 1 + images.length) % images.length; else if (startX - endX > 50) currentImg = (currentImg + 1) % images.length; updateImg(); });

  const bookBtn = modalBody.querySelector(".luxury-btn");
  if (bookBtn) bookBtn.addEventListener("click", () => { selectVehicle(`${car.year} ${car.brand} ${car.model}`); closeModal(); });
